...
## PIPELINE: scRNA-seq raw read analysis
## __current version__
The current version of the pipeline is can not only process raw reads generated by the SPLiT-seq experiment, but also data from 10X genomics technologies. Furthermore, Demultiplexing and UMI collapsing implemented in Python replace the respective tool from the Drop-seq tool box that was used in the old version of the pipeline. Also scripts have been added that allow the conversion between gene_symbols and gene_ids together with a script that parses and corrects .gtf files from Arabidopsis Thaliana.
